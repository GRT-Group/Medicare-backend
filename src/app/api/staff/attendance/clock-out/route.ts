...
export async function POST(req: NextRequest) {
  try {
    ...
    const attendance = await StaffService.clockOut(organizationId, BigInt(employee_id), notes, time)
    ...
    return NextResponse.json({ success: true, message: 'Clocked out successfully', data: serialized }, { status: 200 })
  } catch (error: any) {
